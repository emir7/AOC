...
function getPairs() {
    const fullInput = input.split("\n").map((line) => eval(line)).filter((el) => el);
    const pairs = [];

    for(let i = 0; i < fullInput.length - 1; i+=2) {
        pairs.push([fullInput[i], fullInput[i+1]])
    }
...
function compare(in1, in2) {
    ...
    for(let i = 0; i < in1.length; i++) {
        ...
        if(Array.isArray(in1[i]) && Number.isInteger(in2[i])) {
            const c = compare(in1[i], [in2[i]]);
            if(c != 0) {
                return c;
            }
        } else if(Array.isArray(in2[i]) && Number.isInteger(in1[i])) {
            const c = compare([in1[i]], in2[i])
            if(c != 0) {
                return c;
            }
        } else if(Array.isArray(in1[i]) && Array.isArray(in2[i])){
            const c = compare(in1[i], in2[i]);
            if(c != 0) {
                return c;
            }
...
let okIndexes = [];
for(let i = 0, c = 1; i < pairs.length; i++, c++) {
    const [i1, i2] = pairs[i];
    const check = compare(i1, i2);
...
console.log(okIndexes)
const sum = okIndexes.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
console.log(sum)
